...
happyExpList = Happy_Data_Array.listArray (0,43) ([1344,16464,5,1344,0,21504,1024,8,16384,512,32768,21504,0,84,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseLambda","Expression","Application","Atom","'?'","'-'","'('","')'","VAR","%eof"]
        bit_start = st * 12
        bit_end = (st + 1) * 12
...
action_0 (7) = happyShift action_8
...
happyReduction_1 ((HappyAbsSyn4  happy_var_5) `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn4
		 (Apply happy_var_1 (Lambda happy_var_3 happy_var_5)
	) `HappyStk` happyRest
...
parseLambda tks = happySomeParser where
